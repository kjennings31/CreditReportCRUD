...
namespace JsonParse
{
    ...
    class GetLastRecord
    {
        ...
        public int GetLast()
        {
            ...
            {
                dbconnection.Open();
                string qstring4 = "SELECT TOP 1 * FROM ACCELITASPREPORTS ORDER BY ID DESC";
                using (SqlCommand cmd = new SqlCommand(qstring4))
                {
                    ...
                    using (SqlDataReader reader = cmd.ExecuteReader())
                    {
                        ...
                        {
                            Console.WriteLine("Accelitas Table currently has no records. Starting record insertion process");
